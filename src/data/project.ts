...
export function groupProjectsByYear(posts: CollectionEntry<"projects">[]) {
  return posts.reduce<Record<string, CollectionEntry<"projects">[]>>((acc, post) => {
 ...
 *  Note: This function doesn't filter draft posts, pass it the result of getAllPosts above to do so.
 *  */
export function getUniqueTagsWithCount(posts: CollectionEntry<"projects">[]): [string, number][] {
  return [
